...
class CallCenterOrder(models.Model):
    ...
    ## change in 2 - 9 - 2024 by abdurrahman
    customer_code=fields.Char()
    # customer_code = fields.Integer(string='customer_code',default=333, readonly=True, copy=False, index=True)
    street = fields.Char(
        readonly=True
    )
    street2 = fields.Char(
    ...
    order_available = fields.Selection(
       [('in_hand', 'In_Hand'),
        ...
        string='Order Availability', readonly=True,tracking=1, 
...
class CallCenterLine(models.Model):
    ...
    def _compute_product_description(self):
        ...
        for record in self:
            ...
            record.product_description = new_name
    ...
    #         vals['customer_code'] = self._generate_unique_customer_code()
    #     return super('res.partner', self).create(vals)

    # def _generate_unique_customer_code(self):
    #     while True:
    #         code = random.randint(1, 999999)  # Generate a 6-digit random number
    #         if not self.search([('customer_code', '=', code)]):
    #             return code
