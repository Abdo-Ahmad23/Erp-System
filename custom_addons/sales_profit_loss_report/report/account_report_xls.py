...
class AccountReporsXLS(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.write_rich_string(0, 0, bold, 'Sales Profit & Loss Report', bold, ' | Date From : ',
                                data['form']['from_date'], bold, ' Date To : ', data['form']['to_date'], merge_format)
        object_head_data = self.env[
            'report.sales_profit_loss_report.report_sales_profit_report'].generate_report_values(data['form'])
        for i in range(0, 11):
            if i in [0, 1]:
                width = 30
            else:
                ...
                if obj_data == 'product_dict':
                    ...
                    for rec in object_head_data[obj_data]:
                        ...
                        for new in new_val:
                            ...
                            sheet.write(count, row_count + 4,
                                        str(new_val[new]['unit_price']) + ' ' + self.env.company.currency_id.symbol,
                                        format2)
                            sheet.write(count, row_count + 5,
                                        str(new_val[new]['taxes']) + ' ' + self.env.company.currency_id.symbol, format2)
                            sheet.write(count, row_count + 6,
                                        str(new_val[new]['amt_wo_tax']) + ' ' + self.env.company.currency_id.symbol,
                                        format2)
                            sheet.write(count, row_count + 7, str(round(new_val[new]['cost_price'],
                                                                        2)) + ' ' + self.env.company.currency_id.symbol,
                                        format2)
                            sheet.write(count, row_count + 8, str(round(new_val[new]['gross_pnl'],
                                                                        2)) + ' ' + self.env.company.currency_id.symbol,
                                        format3 if new_val[new]['gross_pnl'] > 0 else format4)
                            count += 1
