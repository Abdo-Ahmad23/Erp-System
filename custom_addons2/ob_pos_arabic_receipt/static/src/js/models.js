...
odoo.define('ob_pos_arabic_receipt.models', function (require) {
    "use strict"
...
//                    }
         async _printReceipt() {
                var self = this;
                if(!self.env.pos.config.enable_arabic_receipt){
...
//                var orderManagementContext = arguments[1]['order'];
                    //if (self.env.pos.proxy.printer) {
                    console.log("xxxxxxxxxxxxxxxxxxxxxxxxx", self.env.pos.config)
                    console.log("xxxxxxxxxxxxxxxxxxxxxxxxx", self.env.pos.config.receipt_design_id)
                        ...
                        if(!orderdate){
                        orderdate = order.validation_date;
                        }
            ...
            constructor() {
                ...
                setTimeout( async function(){
                    if(self.env.pos.config.enable_arabic_receipt){
                        ...
                        console.log(self.env.pos.config.receipt_design_id)
                        var receipt_design_id = self.env.pos.config.receipt_design_id[0]
                        var receipt_design = self.env.pos.db.receipt_by_id[receipt_design_id].receipt_design
                        var order = self.env.pos.get_order();
                        if(order.orderlines.length == 0 ){
                            order = orderManagementContext;
