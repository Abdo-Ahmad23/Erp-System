...
class ImportPosWizard(models.TransientModel):
    ...
    def show_success_msg(self, counter, confirm_rec, skipped_line_no):
        ...
        action = self.env.ref(
            'sh_pos_all_in_one_retail.sh_import_pos_action').read()[0]
                                    ...
                                    if created_pos:
                                        ...
                                        if search_product:
                                            ...
                                            vals.update({'price_subtotal': float(
                                                row[9]) * float(row[7]),
                                                         'price_subtotal_incl': float(row[9]) * float(row[7])})
                                            vals.update(
                ...
                if rec.import_type == 'excel':
                    ...
                    for row in range(sheet.nrows):
                        try:
                            ...
                            if sheet.cell(row, 0).value not in (None, "") and sheet.cell(row, 5).value not in (
                            None, ""):
                                vals = {}
                                ...
                                if created_pos:
                                    ...
                                    if search_product:
                                        ...
                                        if sheet.cell(row, 6).value != '':
                                            ...
                                            vals.update(
                                                {'full_product_name': sheet.cell(row, 6).value})
                                        ...
                                        if sheet.cell(row, 10).value in (None, "") and search_product.taxes_id:
                                            ...
                                            if some_taxes_not_found:
                                                ...
                                                vals.update(
                                                    {'tax_ids': [(6, 0, taxes_list)]})
                                        vals.update(
                                            {'price_subtotal': float(sheet.cell(row, 9).value) * float(sheet.cell(
                                                row, 7).value),
                                             'price_subtotal_incl': float(sheet.cell(row, 9).value) * float(
