...
class ShProductTemplate(models.Model):
    ...
    def _onchange_attribute_line_ids(self):
        if self and self.sh_is_bundle:
            raise ValidationError(_('You can not add variant if product have bundle.'
                                    ))

    @api.onchange('sh_is_bundle')
    def _onchange_is_bundle(self):
        if self and self.attribute_line_ids:
            raise ValidationError(_('You can not add bundle if product have variant.'
                                    ))
...
class Product(models.Model):
    ...
    @api.onchange('sh_is_bundle')
    def _onchange_is_bundle(self):
        if self and self.product_template_attribute_value_ids:
            raise ValidationError(_('You can not add bundle if product have attribute.'
                                    ))
