...
class ShKeyboardKey(models.Model):
    ...
    _description = 'Define keyboard shortcut keys'
...
class ShKeyboardKeyTemp(models.Model):
    ...
    _description = "Keyboard shortcur keys"
...
class ShPosKeyboardShortcut(models.Model):
    ...
    sh_shortcut_screen_type = fields.Selection(
        [('payment_screen', 'Payment Screen'), ('product_screen', 'Product Screen'), (
            'customer_screen', 'Customer Screen'), ('receipt_screen', 'Receipt Screen'),
         ('order_screen', 'Order Screen'), ('all', 'All')], string="Shortcut Screen Type")
    sh_shortcut_screen = fields.Selection(
        [('go_payment_screen', 'Go to Payment Screen'), ('go_customer_Screen', 'Go to Customer Screen'),
         ('go_order_Screen', 'Go to Order Screen'), ('validate_order', 'Validate Order'), ('next_order', 'Next Order'),
         ('go_to_previous_screen', 'Go to Previous Screen'), ('select_quantity_mode', 'Select Quantity Mode'),
         ('select_discount_mode', 'Select Discount Mode'), ('select_price_mode', 'Select Price Mode'),
         ('search_product', 'Search Product'), ('search_order', 'Search Order'), ('add_new_order', 'Add New Order'),
         ('destroy_current_order', 'Destroy Order'), ('delete_orderline', 'Delete OrderLine'),
         ('select_up_orderline', 'Select Up OrderLine'), ('select_down_orderline', 'Select Down OrderLine'),
         ('search_customer', 'Search Customer'), ('select_up_customer', 'Select Up Customer'),
         ('select_down_customer', 'Select Down Customer'), ('set_customer', 'Set Customer'),
         ('edit_customer', 'Edit Customer'), ('save_customer', 'Save Customer'), ('create_customer', 'Create Customer'),
         ('delete_payment_line', 'Delete Payment Line'), ('select_up_payment_line', 'Select Up Payment Line'),
         ('select_down_payment_line', 'Select Down Payment Line'), ('+10', '+10'), ('+20', '+20'), ('+50', '+50'),
         ('select_down_order', 'Select Down Order'), ('select_up_order', 'Select Up Order'),
         ('select_order', 'Select Order'), ('bundle_product_info', 'Show Bundle Product Info'),
         ('pricelist_info', 'Show Pricelist Info'), ('warehouse_info', 'Show Warehouse Info'),
         ('add_line_note', 'Add Line Note'), ('create_so', 'Create Sale Order'),
         ('create_po', 'Create Purchase Order')], string="Shortcut Screen")
...
class PosConfig(models.Model):
    ...
    def default_get(self, fields):
        ...
        temp_key_id = self.env['sh.keyboard.key.temp'].sudo().create(
            {'sh_key_ids': key_id.id})
        key_list.append(temp_key_id.id)
        vals.append((0, 0, {'sh_shortcut_screen': 'create_so',
                            'sh_shortcut_screen_type': 'product_screen', 'sh_key_ids': [(6, 0, key_list)]}))
        key_list = []
        ...
        temp_key_id = self.env['sh.keyboard.key.temp'].sudo().create(
            {'sh_key_ids': key_id.id})
        key_list.append(temp_key_id.id)
        key_id = self.env.ref('sh_pos_all_in_one_retail.sh_keyboard_key_R')
        temp_key_id = self.env['sh.keyboard.key.temp'].sudo().create(
            {'sh_key_ids': key_id.id})
        key_list.append(temp_key_id.id)
        vals.append((0, 0, {'sh_shortcut_screen': 'create_po',
                            'sh_shortcut_screen_type': 'product_screen', 'sh_key_ids': [(6, 0, key_list)]}))
        key_list = []
