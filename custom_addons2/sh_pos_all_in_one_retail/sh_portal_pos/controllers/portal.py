...
class ShPOSCustomerPortal(CustomerPortal):
    ...
    def portal_my_pos(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby='none', **kw):
        ...
        searchbar_filters = {
            ...
            'today': {'label': _('Today'), 'domain': [('date_order', '>=', (formatted_date + " 00:00:00")),
                                                      ('date_order', '<=', (formatted_date + " 24:00:00"))]},
            'week': {'label': _('This week'), 'domain': [('date_order', '>=', date_utils.start_of(today, "week")),
                                                         ('date_order', '<=', date_utils.end_of(today, 'week'))]},
            'month': {'label': _('This month'), 'domain': [('date_order', '>=', date_utils.start_of(today, 'month')),
                                                           ('date_order', '<=', date_utils.end_of(today, 'month'))]},
            'year': {'label': _('This year'), 'domain': [('date_order', '>=', date_utils.start_of(today, 'year')),
                                                         ('date_order', '<=', date_utils.end_of(today, 'year'))]},
            'quarter': {'label': _('This Quarter'),
                        'domain': [('date_order', '>=', quarter_start), ('date_order', '<=', quarter_end)]},
            'last_week': {'label': _('Last week'),
                          'domain': [('date_order', '>=', date_utils.start_of(last_week, "week")),
                                     ('date_order', '<=', date_utils.end_of(last_week, 'week'))]},
            'last_month': {'label': _('Last month'),
                           'domain': [('date_order', '>=', date_utils.start_of(last_month, 'month')),
                                      ('date_order', '<=', date_utils.end_of(last_month, 'month'))]},
            'last_year': {'label': _('Last year'),
                          'domain': [('date_order', '>=', date_utils.start_of(last_year, 'year')),
                                     ('date_order', '<=', date_utils.end_of(last_year, 'year'))]},
        }
    ...
    def portal_my_pos_form(self, pos_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        if report_type in ('html', 'pdf', 'text'):
            return self._show_report(model=POS_sudo, report_type=report_type,
                                     report_ref='sh_pos_all_in_one_retail.action_report_pos_receipt', download=download)
