...
class PosConfig(models.Model):
    ...
    group_disable_hide_orders = fields.Many2one('res.groups', compute='_compute_access_rights',
                                                string='POS - Disable New/Delete Orders')

    group_disable_remove = fields.Many2one('res.groups', compute='_compute_access_rights',
                                           string='POS - Disable Remove Button')

    group_disable_remove_ordeline = fields.Many2one('res.groups', compute='_compute_access_rights',
                                                    string='POS - Disable Remove Button ')
    ...
    def _compute_access_rights(self):
        for rec in self:
            ...
            rec.group_disable_remove_ordeline = self.env.ref(
                'sh_pos_all_in_one_retail.group_disable_remove_ordeline')
