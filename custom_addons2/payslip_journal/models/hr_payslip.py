...
class HrPayslip(models.Model):
    ...
    def _action_create_account_move(self):
        ...
        if any(not payslip.struct_id for payslip in payslips_to_post):
            raise ValidationError(_(
                'One of the contract for these payslips has no structure type.'))
        if any(not structure.journal_id for structure in
               payslips_to_post.mapped('struct_id')):
            raise ValidationError(_(
                'One of the payroll structures has no account journal defined on it.'))
            ...
            for slip_date in slip_mapped_data[journal_id]:  # For each month.
                    ...
                    for line in slip.line_ids.filtered(
                        ...
                        if debit_account_id:  # If the rule has a debit account.
                            ...
                            if not debit_line:
                                if self._context.get('batch'):
                                    debit_line = self.with_context(
                                        {'batch': True})._prepare_line_values(
                                        line,
                                        debit_account_id,
                                        date,
                                        debit,
                        ...
                        if credit_account_id:  # If the rule has a credit account.
                            ...
                            credit_line = self._get_existing_lines(
                                line_ids, line, credit_account_id, debit,
                                credit)

                            if not credit_line:
                                if self._context.get('batch'):
                                    credit_line = self.with_context(
                                        {'batch': True})._prepare_line_values(
                                        line,
                                        credit_account_id,
                                        date,
                ...
                if float_compare(credit_sum, debit_sum,
                    ...
                    existing_adjustment_line = (
                        ...
                        line_id['name'] == _('Adjustment Entry')
                    )
                    ...
                    if not adjust_credit:
                        adjust_credit = {
                            ...
                            'partner_id': False if self._context.get(
                                'batch') else slip.employee_id.address_home_id.id,
                            'account_id': acc_id,
    ...
    def _prepare_line_values(self, line, account_id, date, debit, credit):
        return {
            'name': line.name,
            'partner_id': False if self._context.get(
                'batch') else line.slip_id.employee_id.address_home_id.id,
    ...
    def action_payslip_done(self):
        ...
        self.with_context({'batch': True})._action_create_account_move()
        return res
    ...
    def _onchange_employee(self):
        ...
        self.name = '%s - %s - %s' % (payslip_name, self.employee_id.name or '',
